...
import (
	...
	"github.com/go-go-golems/go-go-agent/cmd/agentbus/pkg/commands"
	"github.com/go-go-golems/go-go-agent/cmd/agentbus/pkg/doc"
...
func init() {
	...
	if err := viper.BindPFlag("agent", rootCmd.PersistentFlags().Lookup("agent")); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind agent flag")
	}
	if err := viper.BindPFlag("project-prefix", rootCmd.PersistentFlags().Lookup("project-prefix")); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind project-prefix flag")
	}
	if err := viper.BindPFlag("redis-url", rootCmd.PersistentFlags().Lookup("redis-url")); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind redis-url flag")
	}
	...
	if err := viper.BindEnv("agent", "AGENT_ID"); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind agent environment variable")
	}
	if err := viper.BindEnv("project-prefix", "PROJECT_PREFIX"); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind project-prefix environment variable")
	}
	if err := viper.BindEnv("redis-url", "REDIS_URL"); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind redis-url environment variable")
	}
}
