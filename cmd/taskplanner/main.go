...
import (
	...
	"github.com/go-go-golems/go-go-agent/cmd/taskplanner/pkg/commands"
...
func init() {
	...
	if err := viper.BindPFlag("database", rootCmd.PersistentFlags().Lookup("database")); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind database flag")
	}
	...
	if err := viper.BindEnv("agent", "AGENT_ID"); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind AGENT_ID env")
	}
	if err := viper.BindEnv("redis-url", "REDIS_URL"); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind REDIS_URL env")
	}
	if err := viper.BindEnv("database", "DATABASE_PATH"); err != nil {
		log.Fatal().Err(err).Msg("Failed to bind DATABASE_PATH env")
	}
}
